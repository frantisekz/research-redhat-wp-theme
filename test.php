...
function leader_name() {
  ...
  $sql = $wpdb->get_results("SELECT post_author FROM wp_posts WHERE post_name='{$post_name}' AND post_type='diplomas'");
...
function testFunction() {
   ...
   $query_drop = "DROP TABLE IF EXISTS wp_filtration;";
...
function wpse_236014_check_fields($errors, $update, $user) {
  ...
  if( empty($_POST['university']) || $_POST['university'] == "Default")
    // add an error message to the WP_Errors object
    $errors->add( 'university_required',__('University is required, please select your university before saving.') );
...
function show_extra_profile_fields( $user ) { ?>
		...
		<tr>
            ...
            <!-- if you are leader show else hide--->
            <span class="description" style="font-weight: 600;">(required)</span>
				</td>

        <td>
            <select name="university" id="university" class="regular-text" title="My Dropdown">
                <?php

                $generic_terms_place = get_terms(['taxonomy' => 'parrent_university', 'hide_empty' => false]);

                echo '<option value="Default"><p>Default</p></option>';
                ...
                ?>
                <option value="Default" <?php selected( 'Default', get_the_author_meta( 'university', $user->ID ) ); ?>>Default</option>
                <option value="Brno University of Technology" <?php selected( 'Brno University of Technology', get_the_author_meta( 'university', $user->ID ) ); ?>>Brno University of Technology</option>
                <option value="Charles University" <?php selected( 'Charles University', get_the_author_meta( 'university', $user->ID ) ); ?>>Charles University</option>
                <option value="Czech Technical University in Prague" <?php selected( 'Czech Technical University in Prague', get_the_author_meta( 'university', $user->ID ) ); ?>>Czech Technical University in Prague</option>
            </select>
        </td>
        ...
        <?php
          ...
          $selected_univ = $_POST['myselectbox'];

          if(!empty($_POST['myselectbox'])){
            global $wpdb;

            $sql3 = $wpdb->get_results("SELECT email FROM wp_pokus WHERE firstname='Tereza'", ARRAY_A);
            $merged_sql = array_merge(...$sql3);
            $univ_db = $merged_sql['email'];

      			$update = "UPDATE wp_pokus SET email='$selected_univ' WHERE firstname='Tereza'";
            $wpdb->query($update);

            $sql = "INSERT INTO wp_pokus (firstname, lastname, email) VALUES ('Pavel', 'Hruska', '{$selected_univ}')";
            $wpdb->query($sql);
        ...
        <form action='' method='post'>
         <select name="myselectbox">
           <?php
             $generic_terms_place = get_terms(['taxonomy' => 'parrent_university', 'hide_empty' => false]);
             foreach ($generic_terms_place as $generic_term_place) {
               if ($email == $generic_term_place->name) {
                 echo '<option selected="selected">' . $_SESSION["univ"] . '</option>';
               } else {
                 echo '<option>' . $generic_term_place->name . '</option>';
               }
             }
           ?>
         </select>
          ...
          <!-- if if you are leader show else hide--->
          <span class="description" style="font-weight: 600;">(required)</span>
  			</td>
  		</tr>
		</table>
